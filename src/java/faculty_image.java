...
public class faculty_image extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // String student_username = (String)
            // request.getSession().getAttribute("student_username");
                ...
                if (!isMultipart) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (item.isFormField()) {
                            ...
                            try {
                                ...
                                File savedFile = new File(getServletConfig().getServletContext().getRealPath("/")
                                        + "images\\" + itemName);
                    ...
                    try {
                        if (A.equals("FF")) {
                            ...
                            st.executeUpdate("update login set image_path='" + itemName + "' where username='"
                                    + faculty_username + "'");
                        ...
                        if (A.equals("AA")) {
                            ...
                            ////                            out.println(itemName);
                            st.executeUpdate(
                                    "update admin set image_path='" + itemName + "' where username='" + uname + "'");
                        ...
                        // Statement st = con.createStatement();
                        ////                            out.println(itemName);
                        // st.executeUpdate("update login set imagepath='"+itemName+"' where
                        ...
                        // con=DriverManager.getConnection("jdbc:mysql://localhost:3306/student_login","root","root");
                        // Statement st = con.createStatement();
                        //// out.println(itemName); / if(user==null)
                        // st.executeUpdate("update login set imagepath='"+itemName+"' where
                        // username='"+uname+"'");
                        //// else / st.executeUpdate("update login set imagepath='"+itemName+"'
                        /// where username='"+user+"'");
                        // response.sendRedirect("update_student_image.jsp");
                        // }
                        // else if(A.equals("F"))
                        ...
                        // Statement st = con.createStatement();
                        // out.println(itemName);
                        // st.executeUpdate("update login set image_path='"+itemName+"' where
                        ...
                        // Statement st = con.createStatement();
                        // out.println(itemName);
                        // st.executeUpdate("update admin set image_path='"+itemName+"' where
                        // username='"+uname+"'");
                        //
                    }
